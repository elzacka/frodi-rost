...
| Attack surface kept closed | No URL schemes, no document types, no `NSUserActivity` (Handoff), no Spotlight indexing, no extensions, no app group. One App Intent, `ToggleRecordingIntent`, «Start eller stopp opptak». It exists for the Action Button: the button runs a shortcut, so the intent is published as an App Shortcut and appears under Fróði røst in the button's picker. The same publication is what makes it reachable from the Shortcuts app, from automations and from Siri; iOS offers no way to admit the button and refuse the rest. It runs without confirmation once microphone access is granted, deliberately, since a confirmation would defeat the button while driving. The compensating control is the system's: iOS shows the orange microphone indicator whenever the app holds the microphone, in the foreground or not, so a recording started by anything but the button is visible whenever the screen is on | `Info.plist`, `ToggleRecordingIntent` |
