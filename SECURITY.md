...
| Background                 | `UIBackgroundModes` is `audio`, so a recording goes on after the screen locks. Nothing else runs without the app in front: transcription needs the key, and the key needs an unlocked device                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `Info.plist`, `IsolationTests`                                           |
...
| Attack surface kept closed | No URL schemes, document types, Handoff, Spotlight indexing, extensions or app group. One App Intent, published as an App Shortcut. `ToggleRecordingIntent` («Start eller stopp opptak») exists so the Action Button can run it; it runs without confirmation once microphone access is granted, because a confirmation would defeat the button while driving, and the compensating control is the system's orange microphone indicator. Publishing it makes it reachable from the Shortcuts app, automations and Siri; iOS offers no way to admit the button and refuse the rest                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `Info.plist`, `ToggleRecordingIntent`                                    |
