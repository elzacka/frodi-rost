...
| Logging | WhisperKit runs with `verbose: false` and `logLevel: .none`. The app writes recording events to the unified log — start, stop and its length, interruptions, a deferred seal — and never content: no audio, no text, no word list. File names are UUIDs. The log stays on the device and is read only with it connected to a Mac | `AudioRecorder.log`, `WhisperTranscriber` |
